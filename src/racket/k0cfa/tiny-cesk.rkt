...
(define (eval-step st)
  ...
  (match expr
    ...
    [`(if ,ec ,et ,ef)
     (define next-kaddr (alloc kaddr label 0))
...
(define (apply-step st)
  ...
  (match k
    ...
    [(letk vars done '() eb pk next-kaddr label)
     (define as (map (λ (i) (alloc kaddr label i)) (range (length vars))))
    ...
    [(letk vars done (cons eh et) eb pk next-kaddr label)
     (define next-next-kaddr (alloc kaddr label 0))
     (define next-k (letk vars (append done (list v)) et eb pk next-kaddr label))
    ...
    [(callcck next-kaddr label)
     ...
     (define addr (alloc kaddr label 0))
    ...
    [(appappk (closure `(,(? lambda-start?) (,xs ...) ,eb) plam) _ _ next-kaddr label)
     (define as (map (λ (i) (alloc kaddr label i)) (range (length xs))))
